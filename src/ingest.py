...
def load_ims_run(
    ...
    snapshot_files = sorted(
        f for f in run_dir.iterdir() if f.is_file() and _parse_timestamp(f.name) is not None
    )
    ...
    if not rows:
        raise FileNotFoundError(
            f"No valid snapshots loaded from {run_dir} (requested bearing_ids={bearing_ids})."
        )
